...
class _JuicerTaskListPageState extends State<JuicerTaskListPage> {
  ...
  Future<void> _fetchTasks() async {
    try {
      List<Task> tasks = await _taskService.fetchTasks(); // Fetch tasks from the service
      setState(() {
        _tasks = tasks;
        _isLoading = false;
      });
  ...
  List<Task> _getFilteredTasks() {
    ...
    return _tasks.where((task) => task.taskType.contains(_filterCriteria!)).toList();
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: Text('Juicer Task List'),
        actions: [
          PopupMenuButton<String>(
            onSelected: _applyFilter,
            itemBuilder: (context) => [
              PopupMenuItem(value: 'Battery', child: Text('Battery Replacement')),
      ...
      body: _isLoading
          ...
          : _errorMessage != null
          ? Center(child: Text(_errorMessage!)) // Show error message if an error occurs
          : _tasks.isEmpty
          ? Center(child: Text('No tasks available')) // Show when there are no tasks
          : ListView.builder(
